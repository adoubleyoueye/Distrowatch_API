venv_path := ~/.venv/distrowatch_api
...
shell:
	@echo 'To activate the venv use source ~/.venv/distrowatch_api/bin/activate . Use deactivate to exit'

lint:
	# starts a pipenv shell, shows autopep8 diff and then fixes the files
	# does the same for isort
	@echo '---Running autopep8---'
	${bin_path}/autopep8 distrowatch_api -r -d
	${bin_path}/autopep8 distrowatch_api -r -i
	@echo '---Running isort---'
	${bin_path}/isort distrowatch_api --diff
	${bin_path}/isort distrowatch_api --atomic
